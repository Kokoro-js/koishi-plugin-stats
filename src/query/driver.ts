...
export class PrometheusDriver {
  ...
  constructor(options: PrometheusConnectionOptions) {
    ...
    if (!options.endpoint) throw "Endpoint is required";
    ...
    if (!!this.options.requestInterceptor)
      this.axiosInstance.interceptors.request.use(
        this.options.requestInterceptor.onFulfilled,
        this.options.requestInterceptor.onRejected,
      );
    if (!!this.options.responseInterceptor)
      this.axiosInstance.interceptors.response.use(
        this.options.responseInterceptor.onFulfilled,
        this.options.responseInterceptor.onRejected,
      );
  }
    ...
    const req = this.axiosInstance.request({
      ...
      auth:
        !!this.options.auth?.username && !!this.options.auth?.password
          ...
          : null,
      proxy:
        !!this.options.proxy?.host && !!this.options.proxy?.port
          ? {
  ...
  private handleResponse<T>(
    ...
    if (err) response = (response as AxiosError).response;
  ...
  private formatTimeToPrometheus(
    input: PrometheusQueryDate | null,
    dEfault?: PrometheusQueryDate,
  ): number {
  ...
  public targets(state?: TargetState): Promise<object> {
    ...
    return this.request("GET", "targets", params).then((data: object) => {
      return {
        activeTargets: !!data["activeTargets"]
          ? data["activeTargets"].map(Target.fromJSON)
          : [],
        droppedTargets: !!data["droppedTargets"]
          ? data["droppedTargets"].map(Target.fromJSON)
          : [],
      };
    });
  }
   ...
   */
  public targetsMetadata(
    matchTarget: SerieSelector,
    metric?: string,
    limit?: number,
  ): Promise<any> {
    const params = {
  ...
  public rules(): Promise<RuleGroup[]> {
    return this.request("GET", "rules").then((data: object) =>
      (!!data["groups"] ? data["groups"] : []).map(RuleGroup.fromJSON),
    );
  }
  ...
  public alerts(): Promise<Alert[]> {
    return this.request("GET", "alerts").then((data: object) =>
      (!!data["alerts"] ? data["alerts"] : []).map(Alert.fromJSON),
    );
  }
